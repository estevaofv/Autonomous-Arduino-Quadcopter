...
void generateCoordinates() { // only minor data filtering
  ...
  if (relAlt > minAlt && SQUAL > minSQUAL) {
    if (dist_1 < maxDist) {
      Xpoint[pointCount] = round((xPos * posMult) + (dist_1 + 10) * cos(radians(yaw - 90)));
      Ypoint[pointCount] = relAlt * -1;
      Zpoint[pointCount] = round((yPos * posMult) + (dist_1 + 10) * sin(radians(yaw - 90)));
      pointCount++;
    }
    if (dist_2 < maxDist) {
      Xpoint[pointCount] = round((xPos * posMult) + (dist_2 + 10) * cos(radians(yaw)));
      Ypoint[pointCount] = relAlt * -1;
      Zpoint[pointCount] = round((yPos * posMult) + (dist_2 + 10) * sin(radians(yaw)));
      pointCount++;
    }
    if (dist_4 < maxDist) {
      Xpoint[pointCount] = round((xPos * posMult) + (dist_4 + 10) * cos(radians(yaw - 180)));
      Ypoint[pointCount] = relAlt * -1;
      Zpoint[pointCount] = round((yPos * posMult) + (dist_4 + 10) * sin(radians(yaw - 180)));
      pointCount++;
    }
  }
